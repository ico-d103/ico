...
import FinanceInvestListItemDetail from "@/components/student/Finance/Invest/FinanceInvestListItemDetail"
import { getFinanceInvestTimeAPI } from "@/api/student/finance/getFinanceInvestTimeAPI"
...
function index() {
	...
	const { pid, item } = router.query


	const getFinanceInvestDetailQuery = useQuery<getFinanceInvestDetailType>(
		["student", "financeInvestDetail", `${pid}`],
		() => getFinanceInvestListItemAPI({ id: Number(pid) }),
		// { staleTime: 200000 },
	)

	const getFinanceInvestTimeQuery = useQuery<getFinanceInvestTimeType>(
		["student", "financeInvestTime"],
		() => getFinanceInvestTimeAPI(),
		// { staleTime: 200000 },
	)
	...
	return (
		<div
			...
			`}
		>
			{pid && getFinanceInvestDetailQuery.data && getFinanceInvestTimeQuery.data && (
				<React.Fragment>
					{!Object.is(Number(item), NaN)  && getFinanceInvestDetailQuery.data.myStocks && getFinanceInvestDetailQuery.data.myStocks.length > Number(item) ?
						<FinanceInvestListItemDetail
						pid={Number(pid)}
						mid={getFinanceInvestDetailQuery.data.myStocks[Number(item)].investId}
						itemIdx={Number(item)}
						data={getFinanceInvestDetailQuery.data}
						tradingStart={getFinanceInvestTimeQuery.data.tradingStart}
						tradingEnd={getFinanceInvestTimeQuery.data.tradingEnd}
					/> :
						<FinanceInvestListItemDetail
							pid={Number(pid)}
							mid={-1}
