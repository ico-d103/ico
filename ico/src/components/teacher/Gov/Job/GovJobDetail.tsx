...
function GovJobDetail({ job, description, wage, backgroundColor, imgUrl, credit }: GovRuleClassDetailProps) {
	...
	return (
		<div ref={wrapperRef} css={outerWrapperCSS} >
			<FormCreator subComp={<GovJobCreate />} frontComp={<GovJobCardCreate />} idx={0} compState={compState} closeComp={closeEditHandler} mainInit={{title: job, content: description}} subInit={{wage, backgroundColor, imgUrl, credit }} initHeight={`${wrapperRef.current && wrapperRef.current.clientHeight}px`} />
